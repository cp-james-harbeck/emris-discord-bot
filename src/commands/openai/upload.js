...
const { uploadDocumentToPinecone } = require("../../handlers/pinecone/pineconeAPI");
...
module.exports = new ChatInputCommand({
  ...
  clientPerms: ["AttachFiles", "ManageMessages"],
  data: {
    description: "Upload a document to LangChain.",
    options: [
      {
        type: 11,
        name: "file",
        description: "A file to upload to LangChain",
        required: true,
      },
    ],
  },
  run: async (client, interaction) => {
    const file = interaction.options.file("file");
    ...
    if (!interaction.member.permissions.has("ADMINISTRATOR")) {
      const noPermissionEmbed = new EmbedBuilder()
        .setColor(0xff0000)
        .setDescription("You don't have permission to upload files.")
        .setTimestamp();
      return await interaction.editReply({
        embeds: [noPermissionEmbed.build()],
      });
    }

    // If the user has admin privileges, upload the file to LangChain
    if (file) {
      try {
        const documentId = await uploadDocumentToPinecone(
          file.content,
          file.name.split(".").pop()
        );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription("File successfully uploaded!")
      .setColor(0x00ff00)
      .setTimestamp()
      .setFooter({
